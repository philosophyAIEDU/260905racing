...
function hash(bytes) { return createHash('sha1').update(`blob ${bytes.length}\0`).update(bytes).digest('hex'); }
let bytes;
try { bytes = await readFile(file); } catch { /* First build downloads the versioned model. */ }
if (!bytes || hash(bytes) !== expected) {
  const response = await fetch('https://raw.githubusercontent.com/mrdoob/three.js/r175/examples/models/gltf/ferrari.glb', { signal: AbortSignal.timeout(60000) });
